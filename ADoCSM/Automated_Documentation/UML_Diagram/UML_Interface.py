...
class UMLClassDiagram_MainInterface(object):
    ...
    def Class_Interface(self, filename_input, output_path, HierarchyLevel, parameter, variables, primitivevariables,
                        complexvariables, methode, block, showPackages, relations, showconstant, showType):
        ...
        ###############################################################################################################################################################################################
        if HierarchyLevel > 0:
            filename_input = ClassConverter.Model_Converter(filename_input, output_path, RelationModel,
                                                            AixLib_path)  # Formats the file and outputs the file
            ##Sets the class with parameters, variables, methods, .., in PlantUML Syntax
            ClassDiagram.put_full_class(self, filename_input, output_path, parameter, variables, primitivevariables,
                                        complexvariables, methode, Package, showPackages, relations, showconstant,
                                        showType)
            DataCheck.filename_exist(self, filename_input)  # Check if the file exists
            for z in Relations.set_relation(filename_input):  # Adds the relations in PlantUMl format to the text file
                ClassDiagram.insert_line(self, filename_input, output_path,
                                         ClassDiagram.number_of_lines(self, filename_input, output_path) - 1,
                                         "\n" + z + "\n")
                continue
        ...
        ###############################################################################################################################################################################################
        if HierarchyLevel > 1:
            for i in Relations.get_Relation(filename_input, self.AixLib_path,
                                            block):  # Find the relations from the child class
                i = i.split(",")
                filename_input = i[0]
                RelationModel = i[1]
                filename_input = ClassConverter.Model_Converter(filename_input, output_path, RelationModel,
                ...
                ##Sets the class with parameters, variables, methods, .., in PlantUML Syntax
                ClassDiagram.put_full_class(self, filename_input, output_path, parameter, variables, primitivevariables,
                                            complexvariables, methode, Package, showPackages, Relation, showconstant,
                                            showType)
                Model1.append(filename_input)
                for z in Relations.set_relation(
                        filename_input):  # Adds the relations in PlantUMl format to the text file
                    ClassDiagram.insert_line(self, filename_input, output_path,
                                             ClassDiagram.number_of_lines(self, filename_input, output_path) - 1,
                                             "\n" + z + "\n")
                    continue
        ...
        if HierarchyLevel > 2:
            for w in range(0, len(Model1), 1):
                for i in Relations.get_Relation(Model1[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    i = i.split(",")
                    if len(i) == 3:
                        filename_input = i[0]
                    ...
                    ##Sets the class with parameters, variables, methods, .., in PlantUML Syntax
                    T = ClassDiagram.put_full_class(self, filename_input, output_path, parameter, variables,
                                                    primitivevariables, complexvariables, methode, Package,
                                                    showPackages, Relation, showconstant, showType)
                    Package = []
            ...
            for x in range(0, len(Model21), 1):
                for z in Relations.set_relation(Model21[x],
                                                AixLib_path):  # Adds the relations in PlantUMl format to the text file
                    ClassDiagram.insert_line(self, Model21[x], output_path,
                                             ClassDiagram.number_of_lines(self, Model21[x], output_path) - 1,
                                             "\n" + z + "\n")
        ...
        if HierarchyLevel > 3:
            for w in range(0, len(Model2), 1):
                for i in Relations.get_Relation(Model2[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
                    i = i.split(",")
                    if len(i) == 3:
                ...
                for x in range(0, len(Model31), 1):
                    for z in Relations.set_relation(
                            Model31[x]):  # Adds the relations in PlantUMl format to the text file
                        ClassDiagram.insert_line(self, Model31[x], output_path,
                                                 ClassDiagram.number_of_lines(self, Model31[x], output_path) - 1,
                                                 "\n" + z + "\n")
        ...
        if HierarchyLevel > 4:
            for w in range(0, len(Model3), 1):
                for i in Relations.get_Relation(Model3[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
                    i = i.split(",")
                    if len(i) == 3:
                ...
                for x in range(0, len(Model41), 1):
                    for z in Relations.set_relation(
                            Model41[x]):  # Adds the relations in PlantUMl format to the text file
                        ClassDiagram.insert_line(Model41[x], output_path,
                                                 ClassDiagram.number_of_lines(Model41[x], output_path) - 1,
                                                 "\n" + z + "\n")
        ...
        if HierarchyLevel > 5:
            for w in range(0, len(Model4), 1):
                for i in Relations.get_Relation(Model4[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
                    i = i.split(",")
                    if len(i) == 3:
            ...
            for x in range(0, len(Model51), 1):
                for z in Relations.set_relation(Model51[x]):  # Adds the relations in PlantUMl format to the text file
                    ClassDiagram.insert_line(self, Model51[x], output_path,
                                             ClassDiagram.number_of_lines(self, Model51[x], output_path) - 1,
                                             "\n" + z + "\n")
        ##Hierarchy Level 7
        ###############################################################################################################################################################################################
        if HierarchyLevel > 6:
            for w in range(0, len(Model5), 1):
                for i in Relations.get_Relation(Model5[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
                    i = i.split(",")
                    if len(i) == 3:
                ...
                for x in range(0, len(Model61), 1):
                    for z in Relations.set_relation(
                            Model61[x]):  # Adds the relations in PlantUMl format to the text file
                        ClassDiagram.insert_line(self, Model61[x], output_path,
                                                 ClassDiagram.number_of_lines(Model61[x], output_path) - 1,
                                                 "\n" + z + "\n")
        ...
        if HierarchyLevel > 7:
            for w in range(0, len(Model6), 1):
                for i in Relations.get_Relation(Model6[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
                    i = i.split(",")
                    if len(i) == 3:
                ...
                for x in range(0, len(Model71), 1):
                    for z in Relations.set_relation(
                            Model71[x]):  # Adds the relations in PlantUMl format to the text file
                        ClassDiagram.insert_line(Model71[x], output_path,
                                                 ClassDiagram.number_of_lines(Model71[x], output_path) - 1,
                                                 "\n" + z + "\n")
        ...
        if HierarchyLevel > 8:
            for w in range(0, len(Model7), 1):
                for i in Relations.get_Relation(Model7[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
                    i = i.split(",")
                    if len(i) == 3:
            ...
            for x in range(0, len(Model81), 1):
                for z in Relations.set_relation(Model81[x]):  # Adds the relations in PlantUMl format to the text file
                    ClassDiagram.insert_line(Model81[x], output_path,
                                             ClassDiagram.number_of_lines(Model81[x], output_path) - 1, "\n" + z + "\n")

        ##Hierarchy Level 10
        ###############################################################################################################################################################################################
        if HierarchyLevel > 9:
            for w in range(0, len(Model8), 1):
                for i in Relations.get_Relation(Model8[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
        ...
        for x in range(0, len(Model91), 1):
            for z in Relations.set_relation(Model91[x]):  # Adds the relations in PlantUMl format to the text file
                ClassDiagram.insert_line(Model91[x], output_path,
                                         ClassDiagram.number_of_lines(Model91[x], output_path) - 1, "\n" + z + "\n")

        ##Hierarchy Level 11
        ###############################################################################################################################################################################################
        if HierarchyLevel > 10:
            for w in range(0, len(Model9), 1):
                for i in Relations.get_Relation(Model9[w], AixLib_path,
                                                block):  # Find the relations from the child class
                    Package = []
                    i = i.split(",")
                    if len(i) == 3:
            ...
            for x in range(0, len(Model11), 1):
                for z in Relations.set_relation(Model11[x]):  # Adds the relations in PlantUMl format to the text file
                    ClassDiagram.insert_line(Model11[x], output_path,
                                             ClassDiagram.number_of_lines(Model11[x], output_path) - 1, "\n" + z + "\n")
...
if __name__ == "__main__":
    ...
    ## Configure Files
    filename_input = r"..\..\Modelica_Library\AixLib\Fluid\Actuators\Valves\ExpansionValves\SimpleExpansionValves\IsenthalpicExpansionValve.mo"
    AixLib_path = r"..\..\Modelica_Library"
    output_path = r"Java_Klassen"
    Model = [filename_input]
    finalData = r"Java_Klassen\Gesamt\Ventil.java"
    finalData2 = r"Java_Klassen\Gesamt\Ventil2.java"
    ...
    Instanz.Class_Interface(filename_input, output_path, HierarchyLevel, parameter, variables, primitivevariables,
                            complexvariables, methode, block, showPackages, relations, showconstant, showType)
