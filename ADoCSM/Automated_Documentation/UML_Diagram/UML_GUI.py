...
from UML_Interface import  UMLClassDiagram_MainInterface
from General import *
import subprocess
...
class Ui_MainWindow(QWidget):
	...
	def ClassDiagram(self):
		...
		print("test")
		print(finalData)
		finalData2 = finalData.replace(".","2.")
		print(finalData2)
		...
		if text == "Class Diagram":
			try:
				...
				print("start") 
				Interface  = UMLClassDiagram_MainInterface(AixLib_path)
				Interface.Class_Interface(filename_input,output_path,HierarchyLevel,parameter,variables,primitivevariables,complexvariables,methode,block,showPackages,Relation,showconstant,showType)
				DataCheck.Appender(self,output_path, finalData)
				Interface.start_end_syntax(finalData,0,"@startuml")
				Interface.start_end_syntax(finalData,Interface.test2(finalData), " \n @enduml")
				Interface.sort(finalData,finalData2)
	...
	def loadFileInput(self):
		fd = QFileDialog()
		fname = fd.getOpenFileName(self, 'Load Input file' ,"..\..\Modelica_Library\AixLib\Fluid\Actuators\Valves\ExpansionValves\SimpleExpansionValves")
		self.label1.setText(fname[0])
		return fname[0]
	def checkloadInput(self):
		return self.label1.text()
	def loadFileOutput(self):
		path = QFileDialog.getExistingDirectory(self,"Choose Output directory", "Java_Klassen")
		self.label3.setText(path)
		return path
	def checkloadFileOutput(self):
		return  self.label3.text()
	def loadFileModelLibrary(self):
		path = QFileDialog.getExistingDirectory(self,"Choose Modellibrary directory","..\..\Modelica_Library")
	...
	def loadFileFinalData(self):
		fd = QFileDialog()
		fname = fd.getOpenFileName(self, 'Set Final file' ,'Java_Klassen\Gesamt\Ventil.java',"(*.java )")
